...
export class AuthorizeService {
    ...
    private async get_schedules(bullet_token: string, web_version: string): Promise<CoopSchedule[]> {
        ...
        try {
            return plainToInstance(CoopScheduleResponse, (await axios.post(url, parameters, { headers: headers })).data, {
    ...
    async authorize(): Promise<AuthorizeResponse> {
        ...
        const documents = await Promise.all(
            Object.values(Setting).map((setting) => getDocs(collection(this.firestore, setting)), limit(10)),
        );
        console.log(documents);
        const database: CoopScheduleDataResponse[] = documents
            .flatMap((document) => document.docs.map((doc) => plainToInstance(CoopScheduleDataResponse, doc.data())))
    ...
    private async get_f(request: CoralRequest, version: string) {
        ...
        try {
            const response = await axios.post(url, parameters, { headers: headers });
            return plainToInstance(IminkResponse, { ...response.data, ...{ request_id: request.request_id } });
        } catch (error) {
            throw new HttpException(error.response.data, error.response.status);
        }
    }
    ...
    private async plain_text(link: SplatoonInkLink): Promise<{ [name: string]: any }> {
        const base_url = 'https://splatoon3.ink/assets/splatnet/v1';
