...
export class BulletTokenRequest {
    @ApiProperty({ example: '3.0.0-2857bc50' })
    @IsString()
    ...
    static fromToken(game_web_token: string, version: string, country: string) {
        return plainToInstance(BulletTokenRequest, {
            country: country,
            token: game_web_token,
            version: version,
        });
    }
}
